...
browser.storage.local.get(null).then(result => {
    browser.runtime.sendMessage({action: 'storage', get: 'workingdb'}).then(result2 => {
        browser.runtime.sendMessage({action: 'storage', get: 'lastRequest'}).then(result3 => {
            var vm = new Vue({
                el: 'content',
                data: {
                ...
                methods: {
                    ...
                    manualScrobble: function() {
                        ...
                        browser.runtime.sendMessage({action: 'start', animeName, episode, src: 'manual'}).catch(error => console.log(error));
                    },
                    nextEp: function() {
                        var request = this.lastRequest;
                        request.episode = parseInt(request.episode) + 1;
                        request.action = 'start';
                        browser.runtime.sendMessage(request).catch(error => console.log(error));
                    }
                },
                mounted: function() {
                    $('loading').hide();
            ...
            setInterval(function() {
                ...
                browser.runtime.sendMessage({action: 'storage', get: 'lastRequest'}).then(res => {
                    vm.lastRequest = res;
                });
            }, 3500);
        });
